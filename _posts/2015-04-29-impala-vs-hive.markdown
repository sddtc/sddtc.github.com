...
<i class="icon-file"></i>Impala相对于Hive所使用的优化技术
...
> * 通过选择合适的数据存储格式可以得到最好的性能（Impala支持多种存储格式（Parquet, Text, Avro, RCFile, SequeenceFile））。
> * 最大使用内存，中间结果不写磁盘，及时通过网络以stream的方式传递。

<i class="icon-file"></i>Impala与Hive的异同
...
摘自[Impala 与 Hive 的比较](http://tech.uc.cn/?p=1803)
